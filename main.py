...
def alarm(msg):
    ...
    while alarm_status or alarm_status2:
        if alarm_status:
            ...
            saying = True
            sound1.play()
            time.sleep(1)  # Add delay to avoid continuous immediate replay
            saying = False
        if alarm_status2:
            print('Yawn call')
            sound2.play()
...
# Replace imutils VideoStream with cv2.VideoCapture for better control
...
def generate_frames():
    ...
    while True:
        ...
        if not ret or frame is None:
            print("Warning: Unable to grab frame from webcam")
            time.sleep(0.1)
            continue
